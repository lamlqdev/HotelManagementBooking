...
const SearchResultPage = () => {
  ...
  const { data: hotelsData, isLoading, isError, error } = useQuery({
    ...
    queryFn: async () => {
      try {
        // Convert sortBy string to backend sort field
        ...
        // If it's a 404 error (no results found), return an empty list with a message from the server
        ...
        // If it's another error, throw the error for React Query to handle
        throw error;
      }
    },
    enabled: !!locationName && !!finalCheckIn && !!finalCheckOut,
  });
  ...
  const handleHotelClick = (hotelId: string) => {
    ...
    // Add amenities to URL if they exist
    if (amenities.length > 0) {
