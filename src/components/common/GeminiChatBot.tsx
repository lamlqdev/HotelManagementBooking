...
import { geminiChatBotApi } from "@/api/gemini-chat-bot/gemini-chat-bot.api";
...
const GeminiChatBot = () => {
  ...
  const handleSend = async () => {
    ...
    try {
      const response = await geminiChatBotApi.generateContent({
        prompt: userMessage,
      });
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50">
            ...
            <div className="flex flex-col gap-2">
              {messages.length === 0 && (
                <div className="text-center text-muted-foreground text-sm bg-secondary/20 py-2 rounded-lg mb-2">
                  {t(
                    "chatbot.welcome",
                    "Xin chào! Tôi là trợ lý Gemini, tôi có thể giúp gì cho bạn?"
                  )}
                </div>
              )}
      ...
      <button
        ...
        aria-label={t("chatbot.open_chat", "Mở trợ lý Gemini")}
      >
        <BsRobot size={22} />
      </button>
    </div>
